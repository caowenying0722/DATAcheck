...
class QuaternionFixer:
    """四元数符号翻转修复器"""

    @staticmethod
    def fix_quaternion_sign(quats: np.ndarray) -> tuple[np.ndarray, int]:
        """
        标准化四元数符号，确保相邻四元数符号一致

        Args:
            quats: 四元数数组 (N, 4)，格式为[x, y, z, w] 或 [w, x, y, z]

        Returns:
            (修复后的四元数, 修复的翻转数量)
        """
        quats_fixed = quats.copy()
        flip_count = 0
        ...
        return quats_fixed, flip_count

    @staticmethod
    def visualize_fix(original_quats: np.ndarray, fixed_quats: np.ndarray,
                      save_path: Path, dataset_name: str = ""):
        """
        可视化修复前后的对比

        Args:
            original_quats: 原始四元数 (N, 4)
            fixed_quats: 修复后的四元数 (N, 4)
            save_path: 保存路径
        ...
        dots_orig = calc_dots(original_quats)
        dots_fixed = calc_dots(fixed_quats)

        # 统计翻转数量
        flips_orig = np.sum(dots_orig < 0)
        flips_fixed = np.sum(dots_fixed < 0)

        # 创建对比图
        fig, axes = plt.subplots(2, 2, figsize=(15, 10))

        # 1. 原始数据的点积
        axes[0, 0].plot(dots_orig, 'b-', linewidth=0.5, alpha=0.7)
        axes[0, 0].axhline(y=0, color='r', linestyle='--', alpha=0.5)
        axes[0, 0].set_title(f'原始数据 - 相邻四元数点积 ({flips_orig}处翻转)', fontweight='bold')
        axes[0, 0].set_ylabel('点积')
        axes[0, 0].set_xlabel('样本索引')
        axes[0, 0].grid(True, alpha=0.3)
        axes[0, 0].set_ylim([-1.1, 1.1])

        # 2. 修复后数据的点积
        axes[0, 1].plot(dots_fixed, 'g-', linewidth=0.5, alpha=0.7)
        axes[0, 1].axhline(y=0, color='r', linestyle='--', alpha=0.5)
        axes[0, 1].set_title(f'修复后数据 - 相邻四元数点积 ({flips_fixed}处翻转)', fontweight='bold')
        axes[0, 1].set_ylabel('点积')
        axes[0, 1].set_xlabel('样本索引')
        axes[0, 1].grid(True, alpha=0.3)
        axes[0, 1].set_ylim([-1.1, 1.1])

        # 3. 翻转位置分布（原始数据）
        flip_indices_orig = np.where(dots_orig < 0)[0]
        if len(flip_indices_orig) > 0:
            axes[1, 0].hist(flip_indices_orig, bins=50, color='red', alpha=0.7, edgecolor='black')
            axes[1, 0].set_title(f'原始数据 - 翻转位置分布 (共{len(flip_indices_orig)}处)', fontweight='bold')
        else:
            axes[1, 0].text(0.5, 0.5, '无翻转', ha='center', va='center',
                          transform=axes[1, 0].transAxes, fontsize=14)
            axes[1, 0].set_title('原始数据 - 翻转位置分布', fontweight='bold')
        axes[1, 0].set_xlabel('样本索引')
        axes[1, 0].set_ylabel('翻转数量')
        ...
        修复统计
        {'='*30}
        总样本数: {len(original_quats)}

        原始数据:
          翻转数量: {flips_orig}
          翻转率:   {flips_orig/len(original_quats)*100:.2f}%

        修复后数据:
          翻转数量: {flips_fixed}
          翻转率:   {flips_fixed/len(fixed_quats)*100:.2f}%

        修复效果:
          修复数量: {flips_orig - flips_fixed}
          成功率:   {(flips_orig-flips_fixed)/flips_orig*100 if flips_orig > 0 else 100:.1f}%
        """
        ...
        plt.suptitle(f'四元数符号修复对比 - {dataset_name}', fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.savefig(save_path, dpi=150, bbox_inches='tight')
        plt.close()

        print(f"  [Fix] 修复前后对比图已保存: {save_path}")
...
class UniversalConverter:
    ...
    def parse_oxiod_gt(self, csv_path: Path) -> GroundTruthData:
        ...
        Note:
            OxIOD GT quaternions define world->device rotation.
            We need device->world, so we invert the quaternions.
        """
        try:
            df = pd.read_csv(csv_path, header=None)
        ...
        # rots = Rotation.from_quat(q_xyzw).inv()
    ...
    def convert_tlio(self):
        ...
        for npy_path in npy_files:
            ...
            # 查找校准文件
            calib_path = npy_path.parent / "calibration.json"
            calibration_files = []
            if calib_path.exists():
                ...
                try:
                    ...
                    t_device_imu = calib_data.get("T_Device_Imu", {})
                    if "UnitQuaternion" in t_device_imu:
                        print(f"  [INFO] Found T_Device_Imu calibration")
                except Exception as e:
                    print(f"  [WARNING] Failed to read calibration.json: {e}")
            ...
            try:
                ...
                # TLIO数据集特殊处理：
                # 时间戳是相对微秒（从0开始），不是Unix时间戳
                # 通过采样间隔判断单位
                sample_interval = np.median(np.diff(t_val))
                print(f"  Time: Sample interval = {sample_interval:.1f}")
                ...
                else:  # 值 < 1e12，可能是秒或相对微秒
                    # 检查采样间隔来判断
                    if sample_interval > 100 and sample_interval < 100000:
                        # 采样间隔在100-100000范围内（微秒）
                        # 说明时间戳也是微秒（相对时间，从0开始）
                        print(f"  Time: Detected Relative Microseconds (TLIO format). Keeping as is.")
                        t_us = t_val.astype(np.int64)
                    elif sample_interval < 1:
                        # 采样间隔小于1秒，可能是秒级时间戳的高采样率数据
                        print(f"  Time: Detected Seconds (high rate). Converting to us.")
                        t_us = (t_val * self.US_PER_SEC).astype(np.int64)
                    else:
                        # 默认为秒级Unix时间戳
                        print(f"  Time: Detected Unix Seconds. Converting to us.")
                        t_us = (t_val * self.US_PER_SEC).astype(np.int64)
                ...
                UnitSerializer(imu_data, gt_data, calibration_files=calibration_files).save(save_dir)
    ...
    def convert_imunet(self):
        ...
        for csv_path in files:
            ...
            try:
                ...
                # GT orientation selection based on device type
                # For ARCore devices (s10, s21, xiaomi): use rv (Android rotation vector)
                # For Tango devices: use ori (Tango SLAM pose)
                # ARCore's ori is in camera/display frame, not IMU body frame
                device_type = 'unknown'
                if 's10' in seq_name.lower() or 's21' in seq_name.lower() or 'xiaomi' in seq_name.lower():
                    device_type = 'arcore'
                elif 'tango' in seq_name.lower():
                    device_type = 'tango'

                if device_type == 'arcore' and 'rv_w' in df.columns:
                    # Use rotation vector (Android sensor frame) for ARCore devices
                    gt_xyzw = df[['rv_x', 'rv_y', 'rv_z', 'rv_w']].values
                    gt_rot = Rotation.from_quat(gt_xyzw)
                    print(f"  Info: Using rv (rotation vector) for ARCore device ({device_type})")
                elif 'ori_w' in df.columns:
                    # Use ori (SLAM pose) for Tango devices
                    gt_xyzw = df[['ori_x', 'ori_y', 'ori_z', 'ori_w']].values
                    gt_rot = Rotation.from_quat(gt_xyzw)
                    if device_type == 'tango':
                        print(f"  Info: Using ori (Tango pose) for Tango device")
                    else:
                        print(f"  Info: Using ori (device type: {device_type})")
                else:
                    gt_rot = Rotation.identity(len(t_us))
                    print(f"  Warning: No GT orientation found, using identity")
    ...
    def convert_ronin(self):
        ...
        FIXES APPLIED:
        1. Apply align_tango_to_body to GT orientation (Tango frame -> IMU body frame)
        2. Reconstruct acceleration from linacce + standard gravity (9.81 m/s²)
        3. Gravity aligned to world Z-axis
        """
        ...
        for h5_path in files:
            ...
            try:
                with h5py.File(h5_path, 'r') as f:
                    ...
                    # FIXED: Use linacce instead of acce
                    # linacce is the linear acceleration (gravity removed)
                    # We will add standard gravity back later
                    if 'linacce' in synced:
                        linacce = synced['linacce'][:]  # (N, 3) m/s^2
                    else:
                        # Fallback to acce if linacce not available
                        print("  Warning: No linacce found, using acce (may have non-standard gravity)")
                        linacce = synced['acce'][:]
                    ...
                    if 'tango_pos' in pose:
                        ...
                        gt_pos = np.zeros_like(linacce)

                    if 'tango_ori' in pose:
                        gt_quat_tango = pose['tango_ori'][:]  # (N, 4) [x, y, z, w] - TANGO frame
                    elif 'ekf_ori' in pose:
                        gt_quat_tango = pose['ekf_ori'][:]
                    else:
                        gt_quat_tango = np.array([[0, 0, 0, 1]] * len(time_s))

                    # 4. FIXED: Apply align_tango_to_body transformation
                    # This converts GT orientation from Tango frame to IMU body frame
                    info_path = h5_path.parent / "info.json"
                    calibration_files = []
                    gt_quat_body = gt_quat_tango  # Default: no alignment

                    if info_path.exists():
                        calibration_files.append(info_path)
                        try:
                            ...
                            if 'align_tango_to_body' in info_data:
                                ...
                                # Apply alignment to ALL GT quaternions
                                # gt_quat_body = R_align * gt_quat_tango
                                R_gt_tango = Rotation.from_quat(gt_quat_tango)
                                R_gt_body = R_align * R_gt_tango
                                gt_quat_body = R_gt_body.as_quat()

                                euler = R_align.as_euler('xyz', degrees=True)
                                print(f"  [FIX] Applied align_tango_to_body: Euler XYZ=[{euler[0]:.1f}, {euler[1]:.1f}, {euler[2]:.1f}]°")
                            else:
                                print("  [WARNING] No align_tango_to_body found, using raw GT (may misalign!)")
                        except Exception as e:
                            print(f"  [WARNING] Failed to read info.json: {e}")
                    else:
                        print("  [WARNING] No info.json found, using raw GT (may misalign!)")
                    ...
                    n_samples = len(t_us)
                    if not (len(linacce) == len(gt_pos) == n_samples):
                        print(f"  Error: Length mismatch! Time:{n_samples}, LinAcce:{len(linacce)}, GT:{len(gt_pos)}")
                        continue

                    # D. FIXED: Reconstruct acceleration with standard gravity
                    # Use GT orientation to add standard gravity (9.81 m/s²) in world frame
                    # Gravity in world frame is always [0, 0, 9.81]
                    gravity_world = np.array([0, 0, 9.81])

                    # Convert GT quaternions to Rotation objects
                    R_gt_body = Rotation.from_quat(gt_quat_body)

                    # For each sample: acce_body = R_gt_body.T @ gravity_world + linacce_body
                    # Or equivalently: gravity_body = R_gt_body.inv().apply(gravity_world)
                    gravity_body = R_gt_body.inv().apply(gravity_world)

                    # Reconstruct full acceleration (in body frame)
                    acce = linacce + gravity_body

                    # Verify gravity magnitude
                    mean_gravity_body = np.mean(gravity_body[:1000], axis=0)  # First 1000 samples
                    gravity_mag = np.linalg.norm(mean_gravity_body)
                    print(f"  [FIX] Reconstructed acceleration:")
                    print(f"        - Gravity magnitude: {gravity_mag:.2f} m/s² (target: 9.81)")
                    print(f"        - Gravity direction (body frame): [{mean_gravity_body[0]:.2f}, {mean_gravity_body[1]:.2f}, {mean_gravity_body[2]:.2f}]")
                    ...
                    gt_rot = Rotation.from_quat(gt_quat_body)  # Use body frame orientation

                    # Package IMU data (with reconstructed acceleration)
                    imu_data = ImuData(
                        t_us=t_us,
                        gyro=gyro,
                        acce=acce,  # Reconstructed: linacce + standard gravity
                        ahrs=ahrs_rot,
                        magn=magn
                    )

                    # Package GT data (with body frame orientation)
    ...
    # 8. XMU Converter
    # ==========================================
    def convert_xmu(self):
        """Convert XMU dataset to standard format with resampling.

        XMU 数据集特点：
        - IMU 数据包含设备时间戳和系统时间戳
        - GT 数据来自 RTAB-LIB，使用 Unix 时间戳，采样率较低（~25Hz）
        - IMU 采样率较高（~200Hz）
        - 需要重采样和对齐到统一频率

        预处理流程：
        1. 使用系统时间戳对齐 IMU 和 GT
        2. 重采样到统一频率（默认 100Hz）
        3. 插值对齐 IMU 和 GT 数据
        """
        print(f"--- Converting XMU from: {self.dataset_root} ---")

        # 导入重采样相关的函数
        from base.interpolate import get_time_series

        # 递归搜索所有包含 imu.csv 的目录
        imu_files = list(self.dataset_root.rglob("imu.csv"))
        if not imu_files:
            print(f"No imu.csv files found in {self.dataset_root}")
            return

        print(f"Found {len(imu_files)} IMU files")

        for imu_path in imu_files:
            unit_dir = imu_path.parent
            seq_name = unit_dir.name

            # 查找对应的 GT 文件 (rtab.csv 或 gt.csv)
            rtab_path = unit_dir / "rtab.csv"
            gt_path = unit_dir / "gt.csv"
            ...
            if rtab_path.exists():
                ...
                print(f"  Skip {seq_name}: No GT file (rtab.csv or gt.csv) found")
                continue

            print(f"\n>>> Processing XMU: {seq_name}")

            try:
                # 1. 读取 IMU 数据
                imu_df = pd.read_csv(imu_path, comment='#')
                if len(imu_df) == 0:
                    print(f"  Skip {seq_name}: Empty IMU data")
                    continue

                # IMU 列: timestamp, w_RS_S_x, w_RS_S_y, w_RS_S_z, a_RS_S_x, a_RS_S_y, a_RS_S_z, q_RS_w, q_RS_x, q_RS_y, q_RS_z, t_system
                imu_cols = imu_df.columns.tolist()
                print(f"  IMU columns: {imu_cols[:5]}... (total {len(imu_cols)})")

                # 提取时间戳
                imu_timestamps = imu_df.iloc[:, 0].values.astype(np.float64)

                # 提取 IMU 数据
                gyro = imu_df.iloc[:, 1:4].values  # w_RS_S_x, w_RS_S_y, w_RS_S_z
                acce = imu_df.iloc[:, 4:7].values  # a_RS_S_x, a_RS_S_y, a_RS_S_z

                # 提取 AHRS 姿态（如果存在）
                # 注意：q_RS 表示从机器人坐标系到传感器坐标系的旋转
                # 我们需要反转它，得到传感器→机器人的姿态作为参考
                if len(imu_df.columns) >= 11:
                    q_wxyz = imu_df.iloc[:, 7:11].values  # q_RS_w, q_RS_x, q_RS_y, q_RS_z
                    # 转换为 xyzw 格式
                    q_xyzw = np.zeros_like(q_wxyz)
                    q_xyzw[:, 0:3] = q_wxyz[:, 1:4]
                    q_xyzw[:, 3] = q_wxyz[:, 0]
                    # 反转四元数：q_RS^T = q_SR (传感器→机器人)
                    q_rs = Rotation.from_quat(q_xyzw)
                    q_sr = q_rs.inv()
                    ahrs = q_sr  # 使用传感器→机器人的姿态作为参考
                else:
                    ahrs = Rotation.identity(len(imu_timestamps))
                ...
                if len(gt_df) == 0:
                    print(f"  Skip {seq_name}: Empty GT data")
                    continue

                # GT 列: timestamp, p_RN_x, p_RN_y, p_RN_z, q_RN_w, q_RN_x, q_RN_y, q_RN_z
                gt_timestamps = gt_df.iloc[:, 0].values.astype(np.float64)
                pos = gt_df.iloc[:, 1:4].values  # p_RN_x, p_RN_y, p_RN_z
                q_gt_wxyz = gt_df.iloc[:, 4:8].values  # q_RN_w, q_RN_x, q_RN_y, q_RN_z

                # 转换为 xyzw 格式
                q_gt_xyzw = np.zeros_like(q_gt_wxyz)
                q_gt_xyzw[:, 0:3] = q_gt_wxyz[:, 1:4]
                q_gt_xyzw[:, 3] = q_gt_wxyz[:, 0]
                gt_rots = Rotation.from_quat(q_gt_xyzw)

                # 3. 时间戳对齐策略
                # XMU 数据集问题：IMU 使用设备时间，GT 使用 Unix 时间戳
                # 解决方案：检查是否存在系统时间戳列，使用它来对齐

                if len(imu_df.columns) >= 12:
                    # 最后一列是 t_system (Unix 时间戳)
                    imu_system_time = imu_df.iloc[:, 11].values.astype(np.float64)
                    print(f"  Found system time column, using for alignment")

                    # 计算 IMU 和 GT 的时间偏移
                    # 找到最接近的 IMU 和 GT 时间戳
                    gt_start_time = gt_timestamps[0]
                    imu_sys_start_time = imu_system_time[0]

                    # 计算相对时间（微秒）
                    t_imu_us = (imu_system_time - imu_sys_start_time).astype(np.int64)
                    t_gt_us = (gt_timestamps - gt_start_time).astype(np.int64)
                else:
                    # 没有系统时间戳，使用设备时间戳
                    print(f"  Warning: No system time column, using device timestamps")
                    t_imu_us = (imu_timestamps - imu_timestamps[0]).astype(np.int64)
                    t_gt_us = (gt_timestamps - gt_timestamps[0]).astype(np.int64)

                # 归一化时间戳（从0开始）
                t_imu_us = self._normalize_timestamp(t_imu_us)
                t_gt_us = self._normalize_timestamp(t_gt_us)

                print(f"  Before resampling:")
                print(f"    IMU: {len(t_imu_us)} samples, duration: {t_imu_us[-1]/1e6:.2f}s")
                print(f"    GT:  {len(t_gt_us)} samples, duration: {t_gt_us[-1]/1e6:.2f}s")

                # 4. 重采样到统一频率
                # 使用 get_time_series 生成统一的时间轴
                # 注意：不要超过 GT 的原始频率，避免过拟合
                ts_us = [t_imu_us, t_gt_us]
                target_freq = 50.0  # 降低到 50Hz，避免对低频 GT 过采样
                t_resampled_us = get_time_series(ts_us, rate=target_freq)

                print(f"  Resampling to {target_freq}Hz: {len(t_resampled_us)} samples")

                # 5. 构造临时数据对象用于插值
                imu_temp = ImuData(t_imu_us, gyro, acce, ahrs, magn, frame="local")
                gt_temp = GroundTruthData(t_gt_us, gt_rots, pos)

                # 6. 插值到统一时间轴
                imu_resampled = imu_temp.interpolate(t_resampled_us)
                gt_resampled = gt_temp.interpolate(t_resampled_us)

                print(f"  After resampling:")
                print(f"    IMU: {len(imu_resampled.t_us)} samples")
                print(f"    GT:  {len(gt_resampled.t_us)} samples")

                # 7. 保存
                # 保留原始目录结构
                rel_dir = unit_dir.relative_to(self.dataset_root)
                save_dir = self.output_root / rel_dir
                UnitSerializer(imu_resampled, gt_resampled).save(save_dir)
                print(f"  Saved to {save_dir} (IMU: {len(imu_resampled.t_us)}, GT: {len(gt_resampled.t_us)})")
    ...
    def run(self):
        if self.dataset_type == "oxiod":
            ...
            self.convert_rnin()
        elif self.dataset_type == "idol":
            self.convert_idol()
        elif self.dataset_type == "tlio":
            self.convert_tlio()
        elif self.dataset_type == "imunet":
            self.convert_imunet()
        elif self.dataset_type == "ronin":
            self.convert_ronin()
        elif self.dataset_type == "xmu":
            self.convert_xmu()
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--type", type=str, default="oxiod", choices=["oxiod", "ridi", "rnin","idol","tlio","imunet","ronin","xmu"], help="Dataset type")
